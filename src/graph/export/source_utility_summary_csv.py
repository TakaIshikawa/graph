"""CSV export for per-source utility score summaries."""
...
def export_source_utility_summary_csv(
    ...
    *,
    ...
    """Return or write utility score statistics grouped by source project."""
...
def _summary_rows(
    ...
    *,
    ...
    for source_project in sorted(groups, key=_sort_key):
        ...
        values = [_utility_value(unit.utility_score) for unit in source_units]
        utilities = [value for value in values if value is not None]
        rows.append(
            {
                "source_project": source_project,
                "unit_count": len(source_units),
                "utility_count": len(utilities),
                "missing_utility_count": len(source_units) - len(utilities),
                "min_utility": _decimal(min(utilities)) if utilities else "",
                "max_utility": _decimal(max(utilities)) if utilities else "",
                "average_utility": _decimal(sum(utilities) / len(utilities)) if utilities else "",
                "low_utility_count": sum(1 for value in utilities if value < low_threshold),
                "high_utility_count": sum(1 for value in utilities if value >= high_threshold),
            }
        )
    return rows
...
def _utility_value(value: object) -> float | None:
