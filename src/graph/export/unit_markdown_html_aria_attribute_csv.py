"""CSV export for Markdown-embedded HTML ARIA attributes."""
...
_ARIA_ATTR_RE = re.compile(r"""(?:^|\s)(aria-[A-Za-z0-9_.:-]+)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+)))?""", re.IGNORECASE)


def export_units_to_markdown_html_aria_attribute_csv(units: Iterable[Mapping[str, Any] | object], path: str | Path | None = None) -> str | dict[str, Any]:
    unit_list = list(units)
    rows = [row for unit in unit_list for row in _rows(unit)]
    rows.sort(key=lambda row: (sort_key(row["unit_id"]), int(row["line_number"]), sort_key(row["tag_name"]), sort_key(row["attribute_name"])))
...
def _rows(unit: Mapping[str, Any] | object) -> list[dict[str, str | int]]:
    ...
    for match in _TAG_RE.finditer(content):
        ...
        for attr_match in _ARIA_ATTR_RE.finditer(raw_attrs):
            value = field_value(next((group for group in attr_match.groups()[1:] if group is not None), ""))
            rows.append(
                {
                    **context,
