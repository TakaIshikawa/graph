"""CSV export for unit monthly cashflow summaries."""
...
_FIELDNAMES = ["year_month", "source_project", "account", "currency", "transaction_count", "debit_total", "credit_total", "net_amount", "first_date", "last_date"]
_UNKNOWN = "Unknown"
...
def export_unit_monthly_cashflow_csv(
    units: Iterable[KnowledgeUnit | Mapping[str, Any]],
    path: str | Path | Any | None = None,
) -> str | dict[str, Any]:
    """Return or write cashflow grouped by calendar month, source, account, and currency."""
...
def _cashflow_rows(units: list[KnowledgeUnit | Mapping[str, Any]]) -> list[dict[str, str | int]]:
    ...
    for unit in units:
        ...
        groups[key]["dates"].append(txn_date)

    rows: list[dict[str, str | int]] = []
    for (year_month, source, account, currency), group in groups.items():
