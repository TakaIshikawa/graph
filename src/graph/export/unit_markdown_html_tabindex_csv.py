"""CSV export for Markdown-embedded HTML tabindex attributes."""
...
def export_units_to_markdown_html_tabindex_csv(units: Iterable[Mapping[str, Any] | object], path: str | Path | None = None) -> str | dict[str, Any]:
    unit_list = list(units)
    rows = [row for unit in unit_list for row in _rows(unit)]
    rows.sort(key=lambda row: (sort_key(row["unit_id"]), int(row["line_number"]), sort_key(row["tag_name"]), sort_key(row["tabindex"])))
...
def _rows(unit: Mapping[str, Any] | object) -> list[dict[str, str | int]]:
    ...
    for match in _TAG_RE.finditer(content):
        ...
        if "tabindex" not in values:
            continue
        value = values["tabindex"]
        parsed = int(value) if _INT_RE.match(value) else None
        rows.append(
            {
                **context,
                ...
                "tabindex": value,
                "tabindex_int": "" if parsed is None else parsed,
                "is_positive": str(parsed is not None and parsed > 0).lower(),
                "is_zero": str(parsed == 0).lower(),
                "is_negative": str(parsed is not None and parsed < 0).lower(),
                "is_invalid": str(parsed is None).lower(),
                "id": values.get("id", ""),
