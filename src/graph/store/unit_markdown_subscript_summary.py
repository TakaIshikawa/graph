"""Summarize Markdown tilde subscript spans."""
...
def summarize_unit_markdown_subscripts(units: Iterable[Any], sample_limit: int = 5) -> dict[str, Any]:
    ...
    return {
        "total_units": total,
        "units_with_subscript": units_with,
        "subscript_count": count,
...
def _spans(content: str) -> list[tuple[int, str]]:
    ...
    for line_number, line in enumerate(content.splitlines(), start=1):
        ...
        for match in _SUBSCRIPT_RE.finditer(line):
            rows.append((line_number, field_value(match.group(1))))
    return rows
