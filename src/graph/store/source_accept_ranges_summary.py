"""Summarize Accept-Ranges headers in sources."""
...
def summarize_source_accept_ranges(sources: Iterable[Mapping[str, Any] | object], sample_limit: int = 5) -> dict[str, Any]:
    source_list = list(sources)
    limit = max(0, sample_limit)
    value_counts: Counter[str] = Counter()
    samples: list[dict[str, str]] = []
    sources_with = empty_value_count = byte_range_source_count = none_source_count = unknown_value_count = 0
    ...
    for index, source in enumerate(source_list):
        ...
        if not found:
            continue
        value = field_value(raw).casefold()
        if not value:
            empty_value_count += 1
            continue
        sources_with += 1
        ...
        if value == "bytes":
            byte_range_source_count += 1
            if len(samples) < limit:
                samples.append({"source_id": sid, "accept_ranges": value})
        elif value == "none":
            none_source_count += 1
        else:
    ...
    return {
        ...
        "sources_with_accept_ranges": sources_with,
        ...
        "byte_range_source_count": byte_range_source_count,
        "none_source_count": none_source_count,
