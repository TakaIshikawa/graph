"""Adapter for Markdown notes with YAML frontmatter."""
...
class MarkdownNotesAdapter(SourceAdapter):
    @property
    def name(self) -> str:
    ...
    def ingest(
        ...
        *,
        ...
        for file_path in files:
            ...
            aliases = self._normalize_aliases(frontmatter.get("aliases"))
    ...
    def _extract_wikilinks(self, body: str) -> list[dict[str, str]]:
        links: list[dict[str, str]] = []
        seen: set[tuple[str, str, str]] = set()
        for match in WIKILINK_RE.finditer(body):
            raw = match.group(1).strip()
            target_part, alias = self._split_once(raw, "|")
            target, heading = self._split_once(target_part.strip(), "#")
            target = target.strip()
            alias = alias.strip()
            heading = heading.strip()
            if not target:
                continue
            key = (target, alias, heading)
            if key in seen:
                continue
            seen.add(key)
            link = {"raw": raw, "target": target}
            if alias:
                link["alias"] = alias
            if heading:
    ...
    def _split_once(self, value: str, separator: str) -> tuple[str, str]:
    ...
    def _source_id(self, relative_path: str) -> str:
        ...
        return f"markdown_notes:{digest}"
