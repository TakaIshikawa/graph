"""Adapter for Plaid-backed transaction CSV exports."""
...
class PlaidTransactionsCsvAdapter(SourceAdapter):
    ...
    def _unit(self, row: dict[str, Any], source_file: str, index: int) -> KnowledgeUnit | None:
        transaction_id = first(row, "transaction_id", "Transaction ID", "Transaction Id")
        date_text = first(row, "date", "Date")
        timestamp = parse_datetime(date_text)
        authorized_date_text = first(row, "authorized_date", "Authorized Date")
        authorized_date = parse_datetime(authorized_date_text)
        name = first(row, "name", "Name")
        merchant_name = first(row, "merchant_name", "Merchant Name")
        amount = parse_money(first(row, "amount", "Amount"))
        currency = first(row, "iso_currency_code", "currency", "Currency")
        ...
        pending = first(row, "pending", "Pending")
        if not any([transaction_id, date_text, authorized_date_text, name, merchant_name, amount is not None, account_id, account_name, category, pending]):
            return None
        ...
        return KnowledgeUnit(
            source_project=SourceProject.PLAID_TRANSACTIONS_CSV,
            ...
            metadata=metadata,
            tags=list(dict.fromkeys(tag for tag in ["plaid", "transaction", account_name, *category, pending] if tag)),
    ...
    def _content(self, metadata: dict[str, Any]) -> str:
        ...
        parts = [
            ...
            f"Pending: {metadata.get('pending')}" if metadata.get("pending") else "",
        ]
