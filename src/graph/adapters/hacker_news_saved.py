...
class HackerNewsSavedAdapter(SourceAdapter):
    ...
    def ingest(
        ...
        *,
        ...
        result = IngestResult()
        requested = set(entity_types) if entity_types is not None else {"saved_item", "submitter"}
    ...
    def _domain_edges(self, domains: list[KnowledgeUnit], saved_items: list[KnowledgeUnit]) -> list[KnowledgeEdge]:
        ...
        for item in saved_items:
            ...
            if not domain_id:
                continue
            digest = hashlib.sha256(f"{item.source_id}|{domain_id}|saved_item_domain".encode("utf-8")).hexdigest()[:24]
            edges.append(
                KnowledgeEdge(
                    id=f"hacker-news-saved-domain-relates-{digest}",
    ...
    def _external_domain(self, url: str) -> str:
        domain = self._domain(url)
        if domain in {"", "news.ycombinator.com"}:
            return ""
        return domain
