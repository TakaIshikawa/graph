...
class AmazonOrdersCsvAdapter(SourceAdapter):
        ...
        for order_id, rows in groups.items():
            ...
            if "return" in allowed:
                ...
                for return_unit in returns:
                    item_source_id = str(return_unit.metadata.get("item_source_id") or "")
                    if "item" in allowed and item_source_id in item_ids:
                        result.edges.append(self._edge(item_source_id, return_unit.source_id))
    ...
    def _units(
        ...
        for index, row in enumerate(rows):
            ...
            return_unit = self._return_unit(order_id, row, item_source_id, str(title), index, source_files, order_date, now)
    ...
    def _return_unit(
        ...
        status = first(row, "Return Status", "Status")
        refund_text = first(row, "Refund Amount", "Refunded Amount", "Refund")
        refund_amount = parse_money(refund_text)
        if not any([return_date_text, reason, status, refund_text]):
            return None
        return_date = parse_datetime(return_date_text)
        asin = first(row, "ASIN", "ASIN/ISBN", "ISBN")
        source_id = digest_source_id("amazon_orders_csv:return", order_id, asin, item_title, index, return_date_text, refund_text, reason, status)
        title = f"Amazon return for {item_title}"
