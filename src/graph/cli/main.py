...
def _plain_search_snippet(value: object) -> str:
    return " ".join(str(value or "").split())


def _do_export_search_csv(
    store: Store,
    path: str | Path,
    query: str,
    *,
            ...
            writer.writerow(
                {
                    ...
                    "snippet": _plain_search_snippet(result.get("snippet", "")),
                }
            )
    return {
        "path": str(output_path),
...
@app.command(name="export-cytoscape")
def export_cytoscape(
    path: Path = typer.Argument(..., help="Destination Cytoscape JSON file path"),
    unit_id: str | None = typer.Option(
        None,
        "--unit-id",
        help="Center unit ID for a focused neighborhood export",
    ...
    """Export Cytoscape.js elements JSON for visualization."""
    store = _get_store()
    try:
...
@app.command(name="export-search-csv")
def export_search_csv(
    path: Path = typer.Argument(..., help="Destination CSV file path"),
    query: str = typer.Argument(..., help="Search query"),
    limit: int = typer.Option(10, "--limit", "-n", help="Max results"),
    mode: str = typer.Option(
        "fulltext", "--mode", "-m", help="Search mode: fulltext | semantic | hybrid"
    ...
    if stats.get("error"):
        typer.echo(stats["error"])
        raise typer.Exit(code=1)
    typer.echo(f"Exported {stats['row_count']} search result rows to {stats['path']}")
